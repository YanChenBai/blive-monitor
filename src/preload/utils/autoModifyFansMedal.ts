...
async function findFansMedal(roomId: string) {
  ...
  for (let page = 1; page <= total; page++) {
    ...
    if (fansMedal && fansMedal.medal.is_lighted === 1)
      return fansMedal

    // 如果已经有灭掉的牌子后面就没必要找了
    if (all.find(item => item.medal.is_lighted === 0) !== undefined)
      return undefined
  }
...
export async function autoModifyFansMedal(roomId: string) {
  ...
  if (medal.curr_weared.target_roomid.toString() === roomId)
    return
  let medalId: number
